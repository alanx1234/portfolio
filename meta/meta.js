...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      const ret = {
        id: commit,
        url: "https://github.com/alanxia/portfolio/commit/" + commit,
        author,
        date,
        time,
        timezone,
...
function updateScatterPlot(data, commits) {
  ...
  xAxisGroup.selectAll("*").remove(); // <--- NEW LINE 1
...
function renderCommitInfo(data, commits) {
  ...
  const maxLines = d3.max(fileLineCounts, d => d[1]) ?? 0;
  const section = d3.select('#stats');

  section.append('h2').text('Summary');

  const dl = section.append('dl').attr('class', 'stats tiles');

  const add = (label, value, abbrHtml = null) => {
    dl.append('dt').html(abbrHtml ?? label);
    dl.append('dd').text(value);
  };

  add('COMMITS', totalCommits);
  add('FILES', files);
  add('TOTAL <abbr title="Lines of code">LOC</abbr>', totalLOC);
...
function renderScatterPlot(data, commits) {
  // Put all the JS code of Steps inside this function
    ...
    xScale = d3.scaleTime().domain(d3.extent(commits, d => d.datetime)).nice();
...
function updateFileDisplay(filteredCommits){
  ...
  let files = d3
    ...
    .map(([name, lines]) => {
      return { name, lines, type: lines[0].type };
    })
